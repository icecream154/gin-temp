...
// CreatToken 生成一个accToken
func (j *JwtSign) CreateAccToken(claims AccClaims) (string, error) {
...
// 解析Token
func (j *JwtSign) ParseToken(tokenString string) (*AccClaims, error) {
	token, err := jwt.ParseWithClaims(tokenString, &AccClaims{}, func(token *jwt.Token) (interface{}, error) {
...
labelHere:
	if claims, ok := token.Claims.(*AccClaims); ok && token.Valid {
		return claims, nil
	} else {
		return nil, errors.New(my_errors.ErrorsTokenInvalid)
	}
...
func (j *JwtSign) RefreshToken(tokenString string, extraAddSeconds int64) (string, error) {
	...
	if AccClaims, err := j.ParseToken(tokenString); err == nil {
		AccClaims.ExpiresAt = time.Now().Unix() + extraAddSeconds
		return j.CreateAccToken(*AccClaims)
	} else {
		return "", err
	}
}
