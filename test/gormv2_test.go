...
func TestCustomeParamsConnMysql(t *testing.T) {
	...
	confPrams := gorm_v2.ConfigParams{
		Write: struct {
			...
			Pass     string
			Charset  string
		}{Host: "127.0.0.1", DataBase: "db_test", Port: 3306, Prefix: "tb_", User: "root", Pass: "DRsXT5ZJ6Oi55LPQ", Charset: "utf8mb4"},
		Read: struct {
			Host     string
			DataBase string
			Port     int
			...
			Pass     string
			Charset  string
		}{Host: "127.0.0.1", DataBase: "db_stocks", Port: 3306, Prefix: "tb_", User: "root", Pass: "DRsXT5ZJ6Oi55LPQ", Charset: "utf8mb4"}}
	...
	if gormDbMysql, err := gorm_v2.GetSqlDriver("mysql", 0, confPrams); err == nil {
		...
		result := gormDbMysql.Exec("update tb_users  set  real_name='Write数据库更新' where   id<=2 ")
